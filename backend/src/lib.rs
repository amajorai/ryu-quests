...
/// This app's plugin id. MUST stay byte-identical to `manifest.json`'s `id`: Core
/// re-checks on every emit that the event's namespace half is the plugin the
/// authenticated caller actually is, so a drift here makes every emit a 403.
const PLUGIN_ID: &str = "@ryu/quests";
/// App events declared in `manifest.json`'s `contributes.hook_events`. Declaring
/// one without firing it puts a dead entry in the user's hook/workflow picker, so
/// each of these has exactly one emit site below.
const EVENT_QUEST_CREATED: &str = "@ryu/quests#quest.created";
const EVENT_QUEST_SUGGESTED: &str = "@ryu/quests#quest.suggested";
const EVENT_QUEST_COMPLETED: &str = "@ryu/quests#quest.completed";
const EVENT_QUEST_CAPTURED: &str = "@ryu/quests#quest.captured";
...
pub trait QuestsHost: Send + Sync {
    ...
    async fn shadow_call(&self, tool: &str, args: serde_json::Value) -> Option<serde_json::Value>;
...
/// What a board item *is*. Only [`QuestKind::Task`] is a todo the judge evaluates;
/// the rest are captures — things the user grabbed while working and wants to keep.
///
/// This split is load-bearing, not cosmetic: a captured note or link is `status:
/// open` forever, so without gating on the kind the detection sweep would judge
/// every capture on every tick and burn a model call each time (see
/// [`QuestEngine::judge_quest_with_context`] and the `sync_backing_job` calls).
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq, Default)]
#[serde(rename_all = "snake_case")]
pub enum QuestKind {
    /// A todo with a completion condition. Judged on the detection schedule.
    #[default]
    Task,
    /// Free text worth keeping (an answer, an explanation, a decision).
    Note,
    /// A URL, kept with the page/app it came from.
    Link,
    /// A prompt to run next. Copied back out into a chat, then checked off.
    Prompt,
    /// A captured code/text fragment, shown verbatim in a monospace block.
    Snippet,
}

impl QuestKind {
    /// Whether the detection engine should judge items of this kind. Everything
    /// except a task is inert: it has no completion condition to evaluate.
    pub fn is_judged(self) -> bool {
        matches!(self, QuestKind::Task)
    }

    /// Parse the wire form, falling back to `Task` for anything unrecognised so a
    /// newer client's kind never hard-errors a capture.
    pub fn from_wire(s: &str) -> Self {
        match s.trim().to_ascii_lowercase().as_str() {
            "note" => QuestKind::Note,
            "link" => QuestKind::Link,
            "prompt" => QuestKind::Prompt,
            "snippet" => QuestKind::Snippet,
            _ => QuestKind::Task,
        }
    }
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            QuestKind::Note => "note",
            QuestKind::Link => "link",
            QuestKind::Prompt => "prompt",
            QuestKind::Snippet => "snippet",
        }
    }
}

/// Where a capture came from, so a kept snippet never becomes an orphan quote.
/// Every field is optional: a capture typed straight into the composer has no
/// source at all, and a capture from a native app has no URL.
// `ToSchema` because `api::CaptureBody::source` is `Option<CaptureSource>`: the
// derive on the body struct is not enough on its own, every type reachable from a
// `request_body` needs its own derive or the schema will not compile.
#[derive(Debug, Clone, Default, Serialize, Deserialize, utoipa::ToSchema)]
pub struct CaptureSource {
    /// The frontmost application ("Claude", "Cursor", "Google Chrome").
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub app: Option<String>,
    /// The window/tab title at capture time.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub title: Option<String>,
    /// The URL, when the capture came from something addressable — this is what
    /// makes a kept answer traceable back to the conversation it came from.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub url: Option<String>,
}

impl CaptureSource {
    /// Whether anything was actually recorded (an all-empty source is dropped
    /// rather than stored, so the UI can test presence instead of every field).
    fn is_empty(&self) -> bool {
        self.app.is_none() && self.title.is_none() && self.url.is_none()
    }
}
...
pub struct Quest {
    pub id: String,
    pub title: String,
    /// What this item is. Defaults to `task` so every row written before captures
    /// existed deserializes unchanged.
    #[serde(default)]
    pub kind: QuestKind,
    /// The captured text, verbatim and multi-line. `title` is a one-line label
    /// (derived from the body for a capture); this is the thing actually kept.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub body: Option<String>,
    /// Where the capture came from. `None` for anything typed in directly.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub source: Option<CaptureSource>,
    /// Last time the user copied this item back out (prompts especially). Drives
    /// the "used" affordance without forcing the item to `done`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub used_at: Option<String>,
    /// Kept at the top of the board.
...
pub struct QuestEngine {
    ...
    http: reqwest::Client,
    /// Raises this app's declared events for other plugins/workflows. Built from
    /// the environment rather than passed in, so the constructor signature (and
    /// therefore every caller) is unchanged; unhosted it no-ops.
    events: ryu_app_events::EventEmitter,
}

impl QuestEngine {
    pub fn new(store: QuestStore, host: Arc<dyn QuestsHost>, http: reqwest::Client) -> Self {
        // Reuse the caller's client so emits ride the same connection pool as the
        // judge call rather than standing up a second one.
        let events = ryu_app_events::EventEmitter::with_client(PLUGIN_ID, http.clone());
        Self {
            store,
            host,
            http,
    ...
    pub async fn create_quest(
        ...
        // Emitted on the persist, BEFORE the backing job: the quest exists for every
        // reader the moment it is stored, so a host whose `sync_backing_job` errors
        // must not silently swallow the event for a quest that is already on the board.
        self.events
            .emit(
                EVENT_QUEST_CREATED,
                serde_json::json!({
    ...
    /// Keep something the user grabbed while working: a captured selection, a
    /// link, a prompt to run next, a note.
    ///
    /// This is the write path behind the global capture gesture, and it is
    /// deliberately forgiving — the caller is a hotkey, not a form. An absent
    /// `kind` is inferred from the body (a bare URL is a link, anything else a
    /// note) and an absent `title` is derived from the body's first line, so a
    /// capture only ever needs the text itself.
    ///
    /// A non-task capture gets **no backing detection job** and is never judged;
    /// see [`QuestKind::is_judged`].
    pub async fn capture(
        &self,
        kind: Option<QuestKind>,
        title: Option<String>,
        body: String,
        ...
        if body.is_empty() {
            return Err("capture body is empty".to_string());
        }
        ...
        let quest = Quest {
            ...
            title,
            kind,
            // A task captured from a selection keeps the selection as its detail
            // rather than its body, so the board renders it as a plain todo.
            body: (!kind.is_judged()).then(|| body.clone()),
            source,
            used_at: None,
            pinned: false,
            detail: kind.is_judged().then(|| body.clone()),
        ...
        // Only a task earns a scheduled judge run. Giving a note or a link one
        // would put an item on the detection schedule that can never complete.
        if quest.kind.is_judged() {
            let interval = self.resolve_interval().await;
            self.host
                .sync_backing_job(&quest.id, &quest.title, &interval, true)?;
        }
    ...
    /// Record that the user copied an item back out (the prompt "I ran this"
    /// signal). `complete` also checks it off, which is how a prompt list is
    /// worked through. Returns `None` when the item does not exist.
    pub async fn mark_used(&self, id: &str, complete: bool) -> Result<Option<Quest>, String> {
        let Some(mut quest) = self.store.get_quest(id).await.map_err(|e| e.to_string())? else {
            return Ok(None);
        };
        let now = chrono::Utc::now().to_rfc3339();
        quest.used_at = Some(now.clone());
        quest.updated_at = now.clone();
        if complete && quest.status == QuestStatus::Open {
    ...
    /// Pin or unpin an item so it sorts to the top of the board.
    pub async fn set_pinned(&self, id: &str, pinned: bool) -> Result<Option<Quest>, String> {
        let Some(mut quest) = self.store.get_quest(id).await.map_err(|e| e.to_string())? else {
            return Ok(None);
        };
        quest.pinned = pinned;
    ...
    /// Read the scratchpad (the always-there brain-dump buffer).
    pub async fn scratchpad(&self) -> Result<String, String> {
        self.store.get_scratchpad().await.map_err(|e| e.to_string())
    }

    /// Overwrite the scratchpad.
    pub async fn set_scratchpad(&self, text: &str) -> Result<(), String> {
        self.store
            .set_scratchpad(text)
            .await
            .map_err(|e| e.to_string())
    ...
    pub async fn judge_quest_with_context(
        ...
        // Captures (notes, links, prompts, snippets) have no completion condition
        // and stay open by design. Judging them would spend a model call per item
        // per tick, forever, and can only ever produce a meaningless verdict.
        if !quest.kind.is_judged() {
            return Ok(None);
        }
        ...
        if auto {
            ...
            // The engine closed this one on its own — the `auto: true` half of the
            // event. The user-driven half is emitted from `complete_quest`.
            self.events
                .emit(
                    EVENT_QUEST_COMPLETED,
                    completed_payload(&quest, true, Some(verdict.confidence)),
                )
                .await;
        } else {
            ...
            if !already {
                ...
                // Inside the `!already` guard on purpose: a detection pass that
                // reproduces the same verdict is not a new suggestion, and firing
                // every tick would make "when Ryu thinks a task is done" a repeating
                // alarm rather than an event.
                self.events
                    .emit(
                        EVENT_QUEST_SUGGESTED,
                        serde_json::json!({
                            "id": quest.id,
                            "title": quest.title,
                            "confidence": verdict.confidence,
    ...
    pub async fn complete_quest(&self, id: &str, detected: bool) -> Result<Option<Quest>, String> {
        ...
        let now = chrono::Utc::now().to_rfc3339();
        // Read the pending suggestion's confidence before it is cleared — when the
        // user is confirming a detection, that number is the only record of how sure
        // the judge was, and the event is the last chance to carry it out.
        let confidence = quest.suggestion.as_ref().map(|s| s.confidence);
        quest.status = QuestStatus::Done;
        quest.completed_at = Some(now.clone());
        ...
        // `auto: false` for both arms: the user acted, whether they checked the quest
        // off themselves (`source: manual`) or confirmed a detection (`source:
        // detected`). Only the engine's own close in `judge_quest_with_context` is auto.
        self.events
            .emit(
                EVENT_QUEST_COMPLETED,
                completed_payload(&quest, false, confidence),
            )
            .await;
        Ok(Some(quest))
    }
    ...
    /// while it is an OPEN TASK — a capture is never judged, so it never carries a
    /// scheduled job either.
    async fn sync_job(&self, quest: &Quest) {
        // A capture never had a job, so there is nothing to enable OR disable —
        // asking would write a permanently-disabled job row per kept item.
        if !quest.kind.is_judged() {
            return;
        }
        let interval = self.resolve_interval().await;
    ...
    async fn gather_context(&self, quest: &Quest) -> Option<String> {
        ...
        if combined.len() > MAX_EVIDENCE_CHARS {
            // Truncate at the nearest char boundary at/below the cap. A bare
            // `String::truncate(MAX_EVIDENCE_CHARS)` PANICS when byte
            // `MAX_EVIDENCE_CHARS` lands in the middle of a multibyte UTF-8 char —
            // which real (non-ASCII) Shadow evidence can hit.
...
pub fn board_columns(quests: &[Quest]) -> serde_json::Value {
    ...
    for q in quests {
        // TASKS ONLY. A capture is `open` forever by design, so letting one through
        // would fill the board widget's Open column with kept links and notes and
        // offer a "Complete" action on something that can never be completed.
        if !q.kind.is_judged() {
            continue;
        }
...
/// The `@ryu/quests#quest.completed` payload, shared by the two completion
/// paths so a consumer sees one shape whoever closed the quest.
///
/// `source` and `auto` answer different questions and both are load-bearing:
/// `source` is *who decided* (the user vs the judge), `auto` is *whether the engine
/// acted alone*. Accepting a suggestion is `detected` but NOT `auto`. `confidence`
/// is absent for a plain manual check-off — there was no judgement to report.
fn completed_payload(quest: &Quest, auto: bool, confidence: Option<u8>) -> serde_json::Value {
    serde_json::json!({
        "id": quest.id,
        "title": quest.title,
        "source": quest.completion_source.map(|s| match s {
            CompletionSource::Manual => "manual",
            CompletionSource::Detected => "detected",
        }),
        "auto": auto,
...
/// Whether a captured body is *just* a URL, in which case it is kept as a link
/// rather than a note. Deliberately strict: text that merely CONTAINS a URL is
/// still a note, because the surrounding prose is the part worth keeping.
fn looks_like_url(body: &str) -> bool {
    let t = body.trim();
...
/// A one-line label for a capture, taken from the first non-empty line of the
/// body. Truncated on a char boundary so multi-byte text never panics.
...
fn parse_verdict(text: &str) -> Verdict {
    ...
    for line in text.lines() {
        ...
        if let Some(rest) = lower.strip_prefix("met:") {
            ...
            // Preserve original casing for the reason text. `idx` indexes into the
            // trimmed+lowercased `lower`, so slice the TRIMMED line (not the raw
            // `line`) — otherwise a REASON line with leading whitespace slices at the
            // wrong offset (idx is relative to the trimmed text, but `line` still
            // carries the leading spaces).
            reason = line.trim()[idx + "reason:".len()..].trim().to_string();
        }
    }
...
mod tests {
    ...
    #[test]
    fn parses_indented_reason_line_without_misslicing() {
        // Regression: a REASON line with leading whitespace (plausible LLM output)
        // used to slice the raw line at an index computed from the TRIMMED string,
        // yielding "N: shipped" instead of "shipped".
        let v = parse_verdict("  MET: yes\n  CONFIDENCE: 88\n  REASON: The build shipped.");
        assert!(v.met);
        assert_eq!(v.confidence, 88);
        assert_eq!(v.reason, "The build shipped.");
    }
    ...
    #[test]
    fn declared_events_are_namespaced_to_this_plugin() {
        // Core rejects an emit whose namespace half is not the calling plugin, so a
        // drift between these constants and the manifest id is a silent 403 at
        // runtime. Cheaper to catch here.
        for event in [
            EVENT_QUEST_CREATED,
            EVENT_QUEST_SUGGESTED,
            EVENT_QUEST_COMPLETED,
        ] {
            // `#`, not `/`: a scoped plugin id (`@ryu/quests`) contains a slash, so
            // the slash cannot also separate the event name.
            let (namespace, name) = event.split_once('#').expect("event id has a namespace");
            assert_eq!(namespace, PLUGIN_ID);
    ...
    #[test]
    fn completed_payload_distinguishes_the_three_completion_paths() {
        let mk = |source: CompletionSource| Quest {
            id: "q1".into(),
            title: "Ship it".into(),
            kind: QuestKind::Task,
            body: None,
        ...
        // Plain check-off: no judgement happened, so no confidence to report.
        let manual = completed_payload(&mk(CompletionSource::Manual), false, None);
        assert_eq!(manual["source"], "manual");
        assert_eq!(manual["auto"], false);
        assert!(manual["confidence"].is_null());
        // Confirming a suggestion is detected but NOT auto — the user still acted.
        let confirmed = completed_payload(&mk(CompletionSource::Detected), false, Some(70));
        assert_eq!(confirmed["source"], "detected");
        assert_eq!(confirmed["auto"], false);
        assert_eq!(confirmed["confidence"], 70);
        // The engine closing it alone is the only `auto` case.
        let auto = completed_payload(&mk(CompletionSource::Detected), true, Some(91));
        assert_eq!(auto["source"], "detected");
        assert_eq!(auto["auto"], true);
        assert_eq!(auto["completed_at"], "later");
    }
