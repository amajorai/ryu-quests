...
use ryu_quests::api::{
    self, CaptureBody, DetectionConfigBody, JudgeBody, ListQuery, PinBody, QuestBody, QuestsCtx,
    ScratchpadBody, UseBody,
};
use ryu_quests::{
    board_columns, CaptureSource, CompletionSource, DetectionMode, Quest, QuestEngine, QuestEvent,
    QuestKind, QuestStatus, QuestStore, QuestsHost, DETECTION_INTERVAL_PREF, DETECTION_MODE_PREF,
    JUDGE_EFFORT_PREF, JUDGE_MODEL_PREF,
};
...
async fn store_upsert_get_roundtrip_and_missing() {
    ...
    assert_eq!(
        store.get_quest("q1").await.unwrap().unwrap().title,
        "renamed"
    );
    // Still a single row.
...
async fn resolve_judge_model_prefers_pref() {
    ...
    assert_eq!(
        engine.pref_get(JUDGE_MODEL_PREF).await.as_deref(),
        Some("my-model")
    );
    engine.pref_set(JUDGE_EFFORT_PREF, "high").await.unwrap();
    assert_eq!(
        engine.pref_get(JUDGE_EFFORT_PREF).await.as_deref(),
        Some("high")
    );
    cleanup(&db);
}
...
async fn api_create_validates_and_creates() {
    ...
    // List reflects it.
    let Json(list) = api::list_quests(State(ctx), axum::extract::Query(ListQuery::default())).await;
...
// ── captures (Copper-style keeps) ───────────────────────────────────────────

#[tokio::test]
async fn capture_infers_link_from_a_bare_url_and_note_otherwise() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host.clone());

    let link = engine
        .capture(None, None, "https://example.com/thread/42".into(), None)
        .await
        .unwrap();
    assert_eq!(link.kind, QuestKind::Link);

    // Text that merely CONTAINS a url is still a note — the prose is the keep.
    let note = engine
        .capture(None, None, "see https://example.com for the fix".into(), None)
        .await
        .unwrap();
    assert_eq!(note.kind, QuestKind::Note);

    // A bare scheme with no host is not a link.
    let bare = engine
        .capture(None, None, "https://".into(), None)
        .await
        .unwrap();
    assert_eq!(bare.kind, QuestKind::Note);
    cleanup(&db);
}

#[tokio::test]
async fn capture_derives_a_title_from_the_first_nonempty_line() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host);
    ...
    let q = engine
        ...
        .await
        .unwrap();
    assert_eq!(q.title, "The retry budget is per-attempt");
    // The body is kept verbatim-ish (trimmed at the ends only), not truncated.
    assert!(q.body.unwrap().contains("second line"));

    // A long first line is truncated on a char boundary (multi-byte safe).
    let long = "é".repeat(400);
    let q = engine.capture(None, None, long, None).await.unwrap();
    assert_eq!(q.title.chars().count(), 121); // 120 + the ellipsis
    cleanup(&db);
}

#[tokio::test]
async fn capture_rejects_an_empty_body() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host);
    assert!(engine
...
#[tokio::test]
async fn capture_keeps_its_source_and_drops_an_all_empty_one() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host);
    ...
    let q = engine
        .capture(
            Some(QuestKind::Note),
            None,
            "an answer worth keeping".into(),
            Some(CaptureSource {
                app: Some("Claude".into()),
                title: Some("Retry budgets".into()),
                url: Some("https://claude.ai/chat/abc".into()),
            }),
        )
        .await
        .unwrap();
    let src = q.source.expect("source kept");
    assert_eq!(src.app.as_deref(), Some("Claude"));
    assert_eq!(src.url.as_deref(), Some("https://claude.ai/chat/abc"));

    let q = engine
        .capture(
            Some(QuestKind::Note),
            None,
            "typed straight in".into(),
            Some(CaptureSource::default()),
        )
        .await
        .unwrap();
...
/// The load-bearing one: a capture is `status: open` forever, so if the sweep did
/// not gate on the kind it would judge every kept note on every tick, forever.
#[tokio::test]
async fn captures_are_never_judged_and_carry_no_backing_job() {
    let gateway = spawn_gateway("MET: yes\nCONFIDENCE: 99\nREASON: done".into()).await;
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new(gateway));
    host.set_pref(DETECTION_MODE_PREF, "auto_all");
    host.set_shadow(
        "shadow__recent_context",
        json!({ "summary": "lots of recent activity" }),
    );
    let engine = engine_with(store, host.clone());

    let note = engine
        .capture(Some(QuestKind::Note), None, "keep this".into(), None)
        .await
        .unwrap();

    // No scheduled detection job was ever asked for.
    assert!(host.jobs().is_empty(), "a capture must not be scheduled");

    // And an explicit judge run is a no-op even with a gateway that says "done".
    assert!(engine.judge_quest(&note.id).await.unwrap().is_none());
    let after = engine.store.get_quest(&note.id).await.unwrap().unwrap();
    assert_eq!(after.status, QuestStatus::Open);
    assert!(after.last_judged_at.is_none());
    cleanup(&db);
}

#[tokio::test]
async fn a_captured_task_is_still_scheduled_and_judged() {
    let gateway = spawn_gateway("MET: yes\nCONFIDENCE: 99\nREASON: shipped".into()).await;
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new(gateway));
    host.set_pref(DETECTION_MODE_PREF, "auto_all");
    host.set_shadow(
        "shadow__recent_context",
        json!({ "summary": "published the release notes" }),
    );
    let engine = engine_with(store, host.clone());
    ...
    let task = engine
        .capture(
            ...
            "cover the routing rewrite".into(),
            None,
        )
        .await
        .unwrap();
    // A task capture puts the selection in `detail`, not `body`, so the board
    // renders it as a plain todo rather than a kept blob.
    assert!(task.body.is_none());
    assert_eq!(task.detail.as_deref(), Some("cover the routing rewrite"));
    assert_eq!(host.jobs(), vec![(task.id.clone(), true)]);

    let verdict = engine.judge_quest(&task.id).await.unwrap().expect("judged");
    assert!(verdict.met);
    cleanup(&db);
}

#[tokio::test]
async fn completing_a_task_disables_its_job_but_a_capture_never_had_one() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host.clone());

    let note = engine
        .capture(Some(QuestKind::Prompt), None, "try this next".into(), None)
        .await
        .unwrap();
    engine.complete_quest(&note.id, false).await.unwrap();
    // `sync_job` runs on completion; it must still ask for nothing for a capture.
    assert!(host.jobs().iter().all(|(id, _)| id != &note.id));
    cleanup(&db);
}

#[tokio::test]
async fn mark_used_records_the_copy_and_can_check_it_off() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host);

    let p = engine
        .capture(Some(QuestKind::Prompt), None, "rewrite the intro".into(), None)
        .await
        .unwrap();
    assert!(p.used_at.is_none());

    // Copying without completing leaves it on the board.
    let used = engine.mark_used(&p.id, false).await.unwrap().unwrap();
    assert!(used.used_at.is_some());
    assert_eq!(used.status, QuestStatus::Open);

    // Copying with `complete` checks it off, attributed to the user.
    let done = engine.mark_used(&p.id, true).await.unwrap().unwrap();
    assert_eq!(done.status, QuestStatus::Done);
    assert_eq!(done.completion_source, Some(CompletionSource::Manual));
    assert!(done.completed_at.is_some());

    assert!(engine.mark_used("nope", true).await.unwrap().is_none());
    cleanup(&db);
}

#[tokio::test]
async fn pinning_round_trips() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host);
    let q = engine
        .capture(Some(QuestKind::Note), None, "pin me".into(), None)
        .await
        .unwrap();
    assert!(!q.pinned);
    assert!(engine.set_pinned(&q.id, true).await.unwrap().unwrap().pinned);
    assert!(!engine.set_pinned(&q.id, false).await.unwrap().unwrap().pinned);
    assert!(engine.set_pinned("nope", true).await.unwrap().is_none());
    cleanup(&db);
}

#[tokio::test]
async fn scratchpad_round_trips_and_starts_empty() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host);
    assert_eq!(engine.scratchpad().await.unwrap(), "");
    engine.set_scratchpad("brain dump\nline two").await.unwrap();
    assert_eq!(engine.scratchpad().await.unwrap(), "brain dump\nline two");
    // Overwrite, not append.
    engine.set_scratchpad("replaced").await.unwrap();
    assert_eq!(engine.scratchpad().await.unwrap(), "replaced");
    cleanup(&db);
}

#[tokio::test]
async fn store_lists_by_kind() {
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store.clone(), host);

    engine
        .create_quest("a task".into(), None, String::new())
        .await
        .unwrap();
    engine
        .capture(Some(QuestKind::Link), None, "https://a.example".into(), None)
        .await
        .unwrap();
    engine
        .capture(Some(QuestKind::Link), None, "https://b.example".into(), None)
        .await
        .unwrap();

    assert_eq!(store.list_quests().await.unwrap().len(), 3);
    assert_eq!(store.list_quests_of_kind("link").await.unwrap().len(), 2);
    assert_eq!(store.list_quests_of_kind("task").await.unwrap().len(), 1);
    assert_eq!(store.list_quests_of_kind("note").await.unwrap().len(), 0);
    cleanup(&db);
}

#[tokio::test]
async fn reopening_the_store_is_idempotent_and_preserves_kinds() {
    // The `kind` column is added by an ALTER that errors "duplicate column name"
    // on every open after the first; that must be swallowed, not propagated.
    let db = temp_db();
    let store = QuestStore::open(db.clone()).unwrap();
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store, host);
    let q = engine
        .capture(Some(QuestKind::Snippet), None, "fn main() {}".into(), None)
        .await
        .unwrap();
    drop(engine);

    let reopened = QuestStore::open(db.clone()).expect("second open must succeed");
    let got = reopened.get_quest(&q.id).await.unwrap().unwrap();
    assert_eq!(got.kind, QuestKind::Snippet);
    assert_eq!(reopened.list_quests_of_kind("snippet").await.unwrap().len(), 1);
    cleanup(&db);
}

#[tokio::test]
async fn a_quest_row_written_before_kinds_existed_reads_back_as_a_task() {
    // Forward-compat guard for the real migration path: rows persisted by the
    // shipped version have no `kind`/`body`/`source` keys in their JSON at all.
    let (store, db) = open_store().await;
    let legacy = json!({
        "id": "q_legacy",
        "title": "Old todo",
    ...
    let quest: Quest = serde_json::from_value(legacy).expect("legacy row deserializes");
    assert_eq!(quest.kind, QuestKind::Task);
    assert!(quest.body.is_none());
    assert!(!quest.pinned);
    store.upsert_quest(&quest).await.unwrap();
    assert_eq!(store.list_quests_of_kind("task").await.unwrap().len(), 1);
    cleanup(&db);
}

// ── captures: HTTP surface ──────────────────────────────────────────────────

#[tokio::test]
async fn api_capture_validates_and_creates() {
    let (ctx, db, _host) = api_engine().await;
    let (code, _) = api::capture_item(
        State(ctx.clone()),
        Json(CaptureBody {
            body: "  ".into(),
            kind: None,
    ...
    let (code, body) = api::capture_item(
        State(ctx.clone()),
        Json(CaptureBody {
            body: "https://example.com/x".into(),
            kind: None,
            title: None,
            source: None,
        }),
    ...
    .await;
    assert_eq!(code, axum::http::StatusCode::OK);
    assert_eq!(body["quest"]["kind"], json!("link"));

    // An unrecognised kind degrades to `task` rather than erroring.
    let (code, body) = api::capture_item(
        State(ctx),
        Json(CaptureBody {
            body: "who knows".into(),
    ...
    .await;
    assert_eq!(code, axum::http::StatusCode::OK);
    assert_eq!(body["quest"]["kind"], json!("task"));
    cleanup(&db);
}

#[tokio::test]
async fn api_list_filters_by_kind() {
    let (ctx, db, _host) = api_engine().await;
    for (kind, body) in [("note", "a note"), ("prompt", "a prompt")] {
        api::capture_item(
            State(ctx.clone()),
            Json(CaptureBody {
                body: body.into(),
    ...
    let Json(all) = api::list_quests(
        State(ctx.clone()),
        axum::extract::Query(ListQuery::default()),
    )
    .await;
    assert_eq!(all["quests"].as_array().unwrap().len(), 2);

    let Json(prompts) = api::list_quests(
        State(ctx.clone()),
    ...
    .await;
    assert_eq!(prompts["quests"].as_array().unwrap().len(), 1);

    // A blank `?kind=` means "no filter", not "kind of empty-string".
    let Json(blank) = api::list_quests(
        State(ctx),
        axum::extract::Query(ListQuery {
            kind: Some("  ".into()),
        }),
    ...
    .await;
    assert_eq!(blank["quests"].as_array().unwrap().len(), 2);
    cleanup(&db);
}

#[tokio::test]
async fn api_use_and_pin_and_scratchpad() {
    let (ctx, db, _host) = api_engine().await;
    let (_, created) = api::capture_item(
        State(ctx.clone()),
        Json(CaptureBody {
            body: "rewrite the intro".into(),
            kind: Some("prompt".into()),
            title: None,
            source: None,
        }),
    ...
    let (code, body) = api::use_item(
        State(ctx.clone()),
        Path(id.clone()),
        Some(Json(UseBody { complete: true })),
    )
    .await;
    assert_eq!(code, axum::http::StatusCode::OK);
    assert_eq!(body["quest"]["status"], json!("done"));

    // A bodyless POST defaults to "copied, not completed".
    let (code, _) = api::use_item(State(ctx.clone()), Path("nope".into()), None).await;
    assert_eq!(code, axum::http::StatusCode::NOT_FOUND);

    let (code, body) = api::pin_item(
        State(ctx.clone()),
        Path(id.clone()),
    ...
    .await;
    assert_eq!(code, axum::http::StatusCode::OK);
    assert_eq!(body["quest"]["pinned"], json!(true));

    let (code, _) = api::pin_item(State(ctx.clone()), Path("nope".into()), None).await;
    assert_eq!(code, axum::http::StatusCode::NOT_FOUND);

    let Json(empty) = api::get_scratchpad(State(ctx.clone())).await;
    assert_eq!(empty["text"], json!(""));
    let (code, _) = api::set_scratchpad(
        State(ctx.clone()),
        Json(ScratchpadBody {
            text: "dump".into(),
        }),
    ...
    .await;
    assert_eq!(code, axum::http::StatusCode::OK);
    let Json(read) = api::get_scratchpad(State(ctx)).await;
    assert_eq!(read["text"], json!("dump"));
    cleanup(&db);
}

#[tokio::test]
async fn board_columns_lists_tasks_only() {
    // The quest-board widget offers a Complete action per card. A capture is
    // `open` forever and can never be completed, so it must not reach a column.
    let (store, db) = open_store().await;
    let host = Arc::new(FakeHost::new("http://127.0.0.1:1".into()));
    let engine = engine_with(store.clone(), host);

    engine
        .create_quest("a real task".into(), None, String::new())
        .await
        .unwrap();
    engine
        .capture(Some(QuestKind::Link), None, "https://a.example".into(), None)
        ...
        .unwrap();
    engine
        .capture(Some(QuestKind::Note), None, "kept this".into(), None)
        .await
        .unwrap();

    let board = board_columns(&store.list_quests().await.unwrap());
    let columns = board["columns"].as_array().unwrap();
    let open = &columns[0];
    assert_eq!(open["status"], json!("open"));
    assert_eq!(open["count"], json!(1));
    assert_eq!(open["quests"][0]["title"], json!("a real task"));
    // And nothing leaked into the other columns either.
    assert_eq!(columns[1]["count"], json!(0));
    assert_eq!(columns[2]["count"], json!(0));
    cleanup(&db);
}
