...
async fn judge_survives_multibyte_evidence_over_cap() {
    ...
    host.set_shadow("shadow.recent_context", json!({ "summary": big }));
    let engine = engine_with(store, host);
    let q = engine
        .create_quest("t".into(), None, String::new())
        .await
...
async fn capture_infers_link_from_a_bare_url_and_note_otherwise() {
    ...
    let note = engine
        .capture(
            None,
            None,
            "see https://example.com for the fix".into(),
            None,
        )
        .await
        .unwrap();
...
async fn mark_used_records_the_copy_and_can_check_it_off() {
    ...
    let p = engine
        .capture(
            Some(QuestKind::Prompt),
            None,
            "rewrite the intro".into(),
            None,
        )
        .await
        .unwrap();
...
async fn reopening_the_store_is_idempotent_and_preserves_kinds() {
    ...
    assert_eq!(
        reopened.list_quests_of_kind("snippet").await.unwrap().len(),
        1
    );
    cleanup(&db);
}
